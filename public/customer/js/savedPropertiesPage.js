const listings = document.querySelector('#savedPropertiesListings');
const action = document.querySelector('#savedProperties > div:nth-child(2) > input');
const actionDropDown = document.querySelector('#savedProperties #action');
...
edit.addEventListener("mouseenter", actionMousEnter);
history.addEventListener("mouseenter", actionMousEnter);
...
edit.addEventListener("mouseleave", actionMousEave);
history.addEventListener("mouseleave", actionMousEave);

    //  HIDES ACTION DROP-DOWN.
function hideActionDropDown(e) {        
    const titleBar = document.querySelector('#savedProperties > div:nth-child(1)');
...
function showActionDropDown() {
    ...
    if(listings.innerHTML == "") {
        edit.style.color = '##e5d7c499';
        edit.removeEventListener("click", toggleEditFunction);
        edit.removeEventListener("mouseenter", actionMousEnter);
    };
...
function toggleEditFunction() {
    const imageContainerPorperty1 = document.querySelector('#savedPropertiesListings > .property:nth-child(1) > .imageContainer');
    ...
    if (imageContainerPorperty1.children.length > 1) {
        ...
        edit.removeEventListener("mouseleave", actionMousEave);
        checkMark.src = "https://niwxujzmwpdhegjlmyfw.supabase.co/storage/v1/object/public/D.T.%20Comia%20Realty%20and%20Marketing/BUYER ICONS AND LOGOS/edit (check).png";
        checkMark.alt = "Check mark";
        checkMark.type = "";

        edit.appendChild(checkMark);
...
edit.addEventListener("click", toggleEditFunction);
...
function toggleHistoryFunction() {        
    ...
    window.location = './historyPropertiesPage.html';

};

history.addEventListener("click", toggleHistoryFunction);

    //  .
async function savedPropertiesListings() {    
    const response = await fetch('https://dt-comia-realty-and-marketing-production.up.railway.app/saved-property', {
        method: 'GET',
        credentials: "include",
    });
    ...
    if(data.userId != undefined) {
        for (let i = 0; i < data.savedPropertyListings.length; i++) {
            for (let j = 0; j < data.propertyListings.length; j++) {  
                if(data.savedPropertyListings[i].property_id == data.propertyListings[j].property_id) {
                    ...
                    if(data.propertyListings[j].status == "SOLD") {
                        const sold = document.createElement('div');
                        const soldp = document.createElement('p');

                        sold.classList.add('sold');
                        soldp.innerHTML = "Oops! THIS LAND HAS BEEN SOLD."; 

                        property.appendChild(sold);
                        sold.appendChild(soldp);
...
savedPropertiesListings().catch(console.error);



    //  FUNCTION FOR DELETING SAVED.
async function deleteAll(propertyIdInput) {
    const response = await fetch('https://dt-comia-realty-and-marketing-production.up.railway.app/delete-All-Saved', {
        method: 'POST',
        headers: {                    
                    'User-Agent': 'undici-stream-example',
                    'Content-Type': 'application/json',
...
function deleteCheckedTrue() {          
    ...
    //  RUNS deleteAllFunction AND UPDATES savedPropertiesListings.
function deleteAllFunction() {
    const restoreDelete = document.querySelector('body > div:nth-child(4) > div:nth-child(4)');

    deleteCheckedTrue();

        // DELAY BY 10ms TO AVOID TRIGGERING savedPropertiesListings IMMEDIATELY.
    setTimeout(() => {
        savedPropertiesListings().catch(console.error);
    }, 10);
