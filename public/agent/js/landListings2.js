...
async function landListings2() {    
    const response = await fetch('https://dt-comia-realty-and-marketing-production.up.railway.app/land-listings-2', {
        method: 'GET',
        credentials: "include",
    });
    const data = await response.json();

    const listings = document.querySelector('#landListings2');
    ...
    for (let i = 0; i < data.landListings.length; i++) {
        ...
        propertyId.value = data.landListings[i].property_id;
        ...
        function removeDeleteFunction() {
            deleteProperty().catch(console.error);
                ...
                // DELAY BY 10ms TO AVOID TRIGGERING landListings2 IMMEDIATELY.
            setTimeout(() => {
        ...
        priceModal.innerHTML = "&#8369;" + data.landListings[i].price;
        ...
        if(data.landListings[i].status == "SOLD") {
